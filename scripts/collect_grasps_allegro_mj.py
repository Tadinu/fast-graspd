...
from mjmanip.robot.allegro import HAND_XML_PATH as ALLEGRO_XML_PATH
...
NUM_BATCH: int = getattr(cfg.collector_config, "batch_size", 1 if VISUALIZE_BATCHED_MODEL else 4)
...
if False:
    gripper_desc_path = "/media/ducthan/376b23a1-5a02-4960-b3ca-24b2fcef8f891/11_MPC/hydrax/hydrax/models/leap_hand/leap_rh_mjx_freejoint.xml"  # ALLEGRO_XML_PATH
else:
    gripper_desc_path = to_absolute_path(
        os.path.join(
            os.path.dirname(__file__),
            "../assets/grippers/allegro_right_simplified/"
            "allegro_right_display.urdf",
        )
    )
...
add_desc = builder.add_urdf if gripper_desc_path.endswith(".urdf") else builder.add_mjcf
for _ in range(NUM_BATCH):
    add_desc(gripper_desc_path,
             # xform=wp.transform(
             #    wp.vec3(0.0, 0.0, 0.0),
...
joint_limit_lower = wp.array(
    ...
    dtype=float,
    device=WP_DEVICE,
    requires_grad=WP_LOSS_DIFFERENTIABLE,
)
...
obj_ang = wp.zeros((NUM_BATCH, NUM_DIRS), dtype=wp.vec3, device=WP_DEVICE, requires_grad=WP_LOSS_DIFFERENTIABLE)
obj_angd = wp.zeros_like(obj_ang, requires_grad=WP_LOSS_DIFFERENTIABLE)

loss = wp.zeros(1, dtype=float, device=WP_DEVICE, requires_grad=WP_LOSS_DIFFERENTIABLE)
...
def optimize_grasp():
    # Reset buffers
    wp.launch(
        kernel=wp_kernel_reset_buffers,
...
# NOTE: Only capture the tape's forward launches of [optimize_grasp()] into [grasp_optimizing_graph]
# For some reason (probably like in Pytorch, backward pass depends on intermediate values or runtime conditions),
# [tape.backward()] if captured will compromise [grasp_optimizing_graph()] launch.
tape = wp.Tape()
with wp.ScopedCapture(device=WP_DEVICE) as grasp_optimizing_capture:
    with tape:
        optimize_grasp()
grasp_optimizing_graph = grasp_optimizing_capture.graph
tape.backward(loss)
# NOTE: tape.gradients (as a dict) will then only contain arrays (their python refs) of green ellipses
...
if gripper_desc_path.endswith(".urdf"):
    render_model_desc_path = to_absolute_path(
        os.path.join(
            os.path.dirname(__file__),
...
if VISUALIZE_BATCHED_MODEL:
    renderer.set_model(model, builder)
else:
...
def model_render(batch_idx: int,
...
def optimize(it):
    ...
    tape.backward(loss)  # NOTE: loss is scalar, so no need to specify `grads`
    # wp.synchronize()
    ...
    wp.capture_launch(apply_grad_capture.graph)

    # Render model
    for batch_idx in range(NUM_BATCH):
        # Hand joints
        model_render(batch_idx,
...
if __name__ == "__main__":
    for b in range(NUM_BATCH_PER_OBJ):
        ...
        if not VISUALIZE_BATCHED_MODEL:
